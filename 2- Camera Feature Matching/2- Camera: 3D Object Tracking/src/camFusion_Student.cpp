...
void computeTTCLidar(std::vector<LidarPoint> &lidarPointsPrev,
  ...
  for (auto it = lidarPointsPrev.begin(); it != lidarPointsPrev.end(); it++) {
    prevPoints.push_back(it->x);
  }
  sort(prevPoints.begin(), prevPoints.end());
  minDist = prevPoints[prevPoints.size() / 5];

  std::vector<double> currPoints;
  for (auto it = lidarPointsCurr.begin(); it != lidarPointsCurr.end(); it++) {
    currPoints.push_back(it->x);
  }
  sort(currPoints.begin(), currPoints.end());
  maxDist = currPoints[currPoints.size() / 5];

  TTC = (minDist * dT) / (minDist - maxDist);

  cout << "Lidar TTC calculation -----" << endl;
  cout << "minXprev: " << minDist << endl;
  cout << "maxXprev: " << maxDist << endl;
  cout << "TTC: " << TTC << endl;
}
...
void matchBoundingBoxes(std::vector<cv::DMatch> &matches,
                        ...
                        DataFrame &currFrame) {
  // Number of bounding boxes - Construct scores matrix
  int prevFrameBbNumber = prevFrame.boundingBoxes.size();
  int currFrameBbNumber = currFrame.boundingBoxes.size();
  int bbScores[prevFrameBbNumber][currFrameBbNumber] = {};

  // iterate over all matches
  for (auto it = matches.begin(); it != matches.end(); it++) {
    cv::KeyPoint prevFrameKpt = prevFrame.keypoints[it->queryIdx];
    cv::Point prevFramepnt = cv::Point(prevFrameKpt.pt.x, prevFrameKpt.pt.y);

    cv::KeyPoint currFrameKpt = currFrame.keypoints[it->trainIdx];
    cv::Point currFramepnt = cv::Point(currFrameKpt.pt.x, currFrameKpt.pt.y);

    std::vector<int> prevBbIDs, currBbIDs;
    // associate each point with  a bounding box

    for (int i = 0; i < prevFrameBbNumber; i++) {
      if (prevFrame.boundingBoxes[i].roi.contains(prevFramepnt))
        prevBbIDs.push_back(i);
    }

    for (int i = 0; i < currFrameBbNumber; i++) {
      if (currFrame.boundingBoxes[i].roi.contains(currFramepnt))
        currBbIDs.push_back(i);
    }

    // Increment the score in the score matrix
    for (int i : prevBbIDs) {
      for (int j : currBbIDs) {
        bbScores[i][j] += 1;
      }
    }
    // End of looping over all matches keypoints and associating them with BB
  }

  for (int i = 0; i < prevFrameBbNumber; i++) {
    int maxScore = 0;
    int bestID;
    for (int j = 0; j < currFrameBbNumber; j++) {
      if (bbScores[i][j] > maxScore) {
        maxScore = bbScores[i][j];
        bestID = j;
      }
    }
