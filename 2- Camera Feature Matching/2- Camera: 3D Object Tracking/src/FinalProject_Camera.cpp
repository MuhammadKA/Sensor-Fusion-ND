...
int main(int argc, const char *argv[]) {
  ...
  string imgPrefix =
      "KITTI/2011_09_26/image_02/data/000000"; // left camera, color (image_02
                                               // is the color version of the
                                               // images not grey scale)
  string imgFileType = ".png";
  int imgStartIndex = 0; // first file index to load (assumes Lidar and camera
                         // names have identical naming convention)
  int imgEndIndex = 18;  // last file index to load
  int imgStepWidth =
      1; // a variable to decrease the frame rate to have lager disparities
         // between keypoints (currently set to 10 fps)
  int imgFillWidth =
  ...
  /*
  If you would use another sequence, do not forget to replace the previous lines
  with the respctive calibration files
  */
  ...
  /* Set it to true at selected positions in the code. If you set it to true
  here, you are going to get a display of a whole bunch of different windows and
  it is hard to keep track what is what */
  ...
  for (size_t imgIndex = 0; imgIndex <= imgEndIndex - imgStartIndex;
    ...
    // push image into data frame buffer (we use the color image not he grey
    // scale one)
    DataFrame frame;
    frame.cameraImg = img;
    ...
    /* data buffer "boundingBoxes" are augmented with rectangle around detected
     * object ==>  this is the output of the detectObjects method */
    ...
    /*
    minZ = -1.5 assumes a road level surface, if you had a sequence with inclied
    road, or you are driving up a steep slope or have road bumps --> This will
    lead to errors in TTC computations minR ranges form 0->1; a point with
    minR=0.0 is still a valid point, only its reflectivity is really low. We
    focus on ego lane here, you can tune the parameters to include other lanes
    */
        ...
        1) // wait until at least two images have been processed
    {
      ...
      if(true)
      {
          ...
          vector<cv::KeyPoint> keyPointsCarBox;
