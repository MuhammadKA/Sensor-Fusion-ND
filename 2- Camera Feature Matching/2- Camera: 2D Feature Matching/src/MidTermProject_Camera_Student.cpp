...
int main(int argc, const char *argv[]) {
  ...
  vector<DataFrame> dataBuffer; // list of data frames which are held in memory
                                // at the same time
  // DataFrame dataBuffer[dataBufferSize];
  ...
  for (size_t imgIndex = 0; imgIndex <= imgEndIndex - imgStartIndex;
    ...
    vector<cv::KeyPoint>
        keypointsCarBox; // create empty feature list for current image
    if (bFocusOnVehicle) {
      for (auto it = keypoints.begin(); it < keypoints.end(); it++) {
        if (vehicleRect.contains((*it).pt))
          keypointsCarBox.push_back(*it);
      }
      keypoints = keypointsCarBox;
    }
    //// EOF STUDENT ASSIGNMENT
        ...
        1) // wait until at least two images have been processed
    {
      ...
      std::cout << "-------------> No. of matches: " << matches.size() << endl;
      //// EOF STUDENT ASSIGNMENT
