...
void ClusterHelper(const std::vector<std::vector<float>> &points, KdTree *tree,
                   float distanceTol, int curr_index,
  ...
  for (int i : nearby) {
    if (!processed[i])
      ClusterHelper(points, tree, distanceTol, i, processed, cluster);
  }
}
